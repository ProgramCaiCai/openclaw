...
describe("dispatchTelegramMessage draft streaming", () => {
  ...
  it("keeps streamed final text and delivers error payload as a follow-up message", async () => {
    const draftStream = createDraftStream(999);
    createTelegramDraftStream.mockReturnValue(draftStream);
    dispatchReplyWithBufferedBlockDispatcher.mockImplementation(
      async ({ dispatcherOptions, replyOptions }) => {
        await replyOptions?.onPartialReply?.({ text: "Hello from stream" });
        await dispatcherOptions.deliver({ text: "Hello from stream" }, { kind: "final" });
        await dispatcherOptions.deliver(
          { text: "⚠️ Tool failed: forbidden", isError: true },
          { kind: "final" },
        );
        return { queuedFinal: true };
      },
    ...
    expect(deliverReplies).toHaveBeenCalledWith(
      expect.objectContaining({
        replies: [expect.objectContaining({ text: "⚠️ Tool failed: forbidden", isError: true })],
      }),
    );
    expect(draftStream.clear).not.toHaveBeenCalled();
    expect(draftStream.stop).toHaveBeenCalled();
  ...
  it("clears stale preview when a non-error final is sent normally before an error follow-up", async () => {
    const draftStream = createDraftStream(999);
    createTelegramDraftStream.mockReturnValue(draftStream);
    const longText = "x".repeat(5000);
    dispatchReplyWithBufferedBlockDispatcher.mockImplementation(
      async ({ dispatcherOptions, replyOptions }) => {
        ...
        await dispatcherOptions.deliver({ text: longText }, { kind: "final" });
        await dispatcherOptions.deliver(
          { text: "⚠️ Tool failed after send", isError: true },
          { kind: "final" },
        );
        return { queuedFinal: true };
      },
    ...
    expect(deliverReplies).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({
        replies: [expect.objectContaining({ text: "⚠️ Tool failed after send", isError: true })],
      }),
    );
    expect(draftStream.clear).toHaveBeenCalledTimes(1);
    expect(draftStream.stop).toHaveBeenCalled();
