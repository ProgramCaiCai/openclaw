...
vi.mock("../auto-reply/inbound-debounce.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    createInboundDebouncer: (opts: Record<string, unknown>) => {
...
describe("telegram debounce flush isolation", () => {
  it("falls back to per-entry processing when combined flush fails", async () => {
    ...
    const processMessage = vi.fn(async (ctx: unknown) => {
    ...
    registerTelegramHandlers({
      ...
      accountId: "default",
      bot: { on: () => undefined, api: {} } as unknown,
      opts: { token: "tok" },
      runtime: { log: vi.fn(), error: vi.fn(), exit: () => undefined } as unknown,
      mediaMaxBytes: 1024 * 1024,
      telegramCfg: {} as unknown,
      groupAllowFrom: [],
      resolveGroupPolicy: () => ({ allowlistEnabled: false, allowed: true }),
      resolveTelegramGroupConfig: () => ({}),
      shouldSkipUpdate: () => false,
      processMessage,
      logger: { info: vi.fn(), error: vi.fn(), warn: vi.fn(), debug: vi.fn() } as unknown,
    });
