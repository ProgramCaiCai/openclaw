    ...
    enqueueGlobal(async () => {
      ...
      try {
        while (true) {
          ...
          if (!aborted && proactivePromptTokens >= proactiveThreshold) {
            log.info(
              `[near-limit-compaction] promptTokens=${proactivePromptTokens} threshold=${proactiveThreshold} ctx=${contextWindowTokens} reserve=${reserveTokens}; attempting best-effort compaction`,
            );
            try {
              const compactResult = await compactEmbeddedPiSessionDirect({
                sessionId: params.sessionId,
