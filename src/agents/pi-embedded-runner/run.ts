...
} from "../incomplete-openai-responses-retry.js";
import {
...
import { resolveEmbeddedPiIncompleteRunMaxSilentRetries } from "../pi-project-settings.js";
...
export async function runEmbeddedPiAgent(
  ...
  return enqueueSession(() =>
    enqueueGlobal(async () => {
      ...
      try {
        const manageGlobalLifecycle = isIncompleteOpenAiResponsesRun({
          api: model.api,
          payloads: undefined,
        ...
        const emitManagedLifecycleStart = () => {
          if (!manageGlobalLifecycle) {
            return;
          }
          emitAgentEvent({
            runId: params.runId,
        ...
        const emitManagedLifecycleEnd = (params_: { aborted?: boolean; stopReason?: string }) => {
          if (!manageGlobalLifecycle) {
            return;
          }
          emitAgentEvent({
            ...
            data: {
              ...
              startedAt: lifecycleStartedAt,
              endedAt: Date.now(),
              aborted: params_.aborted ?? false,
              stopReason: params_.stopReason,
            },
          });
        };
        const emitManagedLifecycleError = (error: unknown) => {
          if (!manageGlobalLifecycle) {
            return;
          }
          emitAgentEvent({
        ...
        const returnManagedResult = (result: EmbeddedPiRunResult): EmbeddedPiRunResult => {
          emitManagedLifecycleEnd({
            aborted: result.meta.aborted,
            stopReason: result.meta.stopReason,
          });
          return result;
        };
        const incompleteOpenAiResponsesMaxSilentRetries =
          resolveEmbeddedPiIncompleteRunMaxSilentRetries(params.config);
        ...
        try {
          while (true) {
            ...
            const attempt = await runEmbeddedAttempt({
              ...
              suppressGlobalLifecycleEvents: manageGlobalLifecycle,
            ...
            if (incompleteOpenAiResponsesRun || emptyAssistantShellRun) {
              ...
              if (incompleteOpenAiResponsesRetryCount < incompleteOpenAiResponsesMaxSilentRetries) {
                incompleteOpenAiResponsesRetryCount += 1;
                currentPrompt = retryBaselineRestore.restored
                  ? params.prompt
                  : buildIncompleteOpenAiResponsesFallbackPrompt();
                log.warn(
          ...
          emitManagedLifecycleError(error);
          throw error;
        }
      } finally {
