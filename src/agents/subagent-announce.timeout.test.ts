...
describe("subagent announce timeout config", () => {
  ...
  it("regression, busy requester completions hand accepted runs to the queue", async () => {
    ...
    const queuedWaitCall = findGatewayCall((call) => call.method === "agent.wait");
    expect(completionDirectAgentCall?.expectFinal).toBe(false);
    expect(queuedWaitCall?.params?.runId).toBe("announce-run-1");
    expect(queuedWaitCall?.params?.timeoutMs).toBeGreaterThan(0);
    expect(queuedWaitCall?.params?.timeoutMs).toBeLessThan(90_000);
    expect(queuedWaitCall?.timeoutMs).toBe(queuedWaitCall?.params?.timeoutMs);
  });
