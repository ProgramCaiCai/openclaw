...
const announceLog = createSubsystemLogger("agents/subagent-announce");
...
async function maybeQueueSubagentAnnounce(params: {
  ...
  if (shouldSteer) {
    steerAccepted = await queueEmbeddedPiMessage(sessionId, params.triggerMessage);
    steerRejected = !steerAccepted;
    const fallbackPath =
      steerAccepted && queueSettings.mode !== "steer-backlog"
        ? "none"
        : isActive
          ? "enqueueAnnounce"
          : "directAnnounce";
    announceLog.debug(
      `subagent announce steer result: sessionId=${sessionId} queueMode=${queueSettings.mode} steerAccepted=${steerAccepted} steerRejected=${steerRejected} fallbackPath=${fallbackPath}`,
    );
    if (steerAccepted && queueSettings.mode !== "steer-backlog") {
      return "steered";
    }
  }
  ...
  if (isActive && (shouldFollowup || queueSettings.mode === "steer")) {
    announceLog.debug(
      `subagent announce enqueue: sessionId=${sessionId} queueMode=${queueSettings.mode} steerAccepted=${steerAccepted} steerRejected=${steerRejected} fallbackPath=enqueueAnnounce`,
    );
  ...
  if (shouldSteer) {
    announceLog.debug(
      `subagent announce direct delivery: sessionId=${sessionId} queueMode=${queueSettings.mode} steerAccepted=${steerAccepted} steerRejected=${steerRejected} fallbackPath=directAnnounce`,
    );
  }
  return "none";
}
