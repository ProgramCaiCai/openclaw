...
describe("subagent announce formatting", () => {
  ...
  it("ignores non-assistant messages when resolving fallback output", async () => {
    const { runSubagentAnnounceFlow } = await import("./subagent-announce.js");
    chatHistoryMock.mockResolvedValueOnce({
      messages: [
        {
        ...
        {
          role: "user",
          content: [{ type: "text", text: "sensitive user prompt" }],
        },
      ],
    });
    readLatestAssistantReplyMock.mockResolvedValue("");
    ...
    await runSubagentAnnounceFlow({
      childSessionKey: "agent:main:subagent:worker",
      childRunId: "run-user-fallback-ignore",
      requesterSessionKey: "agent:main:main",
      requesterDisplayKey: "main",
      ...defaultOutcomeAnnounce,
      waitForCompletion: false,
    ...
    expect(msg).not.toContain("sensitive user prompt");
  });
  ...
  it("falls back to durable announce queue when steer is rejected", async () => {
    const { runSubagentAnnounceFlow } = await import("./subagent-announce.js");
    embeddedRunMock.isEmbeddedPiRunActive.mockReturnValue(true);
    embeddedRunMock.isEmbeddedPiRunStreaming.mockReturnValue(true);
    embeddedRunMock.queueEmbeddedPiMessage.mockResolvedValue(false);
    sessionStore = {
      "agent:main:main": {
  ...
  it("keeps steer-backlog delivery durable even when steer is accepted", async () => {
    const { runSubagentAnnounceFlow } = await import("./subagent-announce.js");
    embeddedRunMock.isEmbeddedPiRunActive.mockReturnValue(true);
    embeddedRunMock.isEmbeddedPiRunStreaming.mockReturnValue(true);
    embeddedRunMock.queueEmbeddedPiMessage.mockResolvedValue(true);
    sessionStore = {
      "agent:main:main": {
        sessionId: "session-steer-backlog",
        lastChannel: "whatsapp",
        lastTo: "+1555",
        lastAccountId: "kev",
