...
describe("runReplyAgent transient HTTP retry", () => {
  ...
  it("retries once for OpenAI server_error payload text", async () => {
    vi.useFakeTimers();
    runEmbeddedPiAgentMock
      .mockRejectedValueOnce(
        new Error(
      ...
      .mockResolvedValueOnce({
        payloads: [{ text: "Recovered from server_error" }],
        meta: {},
      });
    ...
    const payload = Array.isArray(result) ? result[0] : result;
    expect(payload?.text).toContain("Recovered from server_error");
  });
});
